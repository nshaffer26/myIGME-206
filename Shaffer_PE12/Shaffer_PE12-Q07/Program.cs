    ...
    class PeopleApp
    {
                            ...
                            if (thisPerson.GetType() == typeof(Student))
                            {
                                ...
                                foreach(string courseCode in student.courseCodes)
                                {
                                    ...
                                    if(thisCourse != null)
                                    {
                                        ...
                                        Console.WriteLine(thisCourse.courseCode);
                                        Console.ForegroundColor = ConsoleColor.White;
                                        Console.BackgroundColor = ConsoleColor.Black;
                                        Console.WriteLine(thisCourse.description + "\n");

                                        // Course days of the week
                                        foreach(DayOfWeek day in Enum.GetValues(typeof(DayOfWeek)).Cast<DayOfWeek>().ToList())
                                        {
                                            Console.Write("|"); 
                                            if(thisCourse.schedule.daysOfWeek.Contains(day))
                                            {
                                                Console.Write(" ");
                                                Console.ForegroundColor = ConsoleColor.Black;
                                                Console.BackgroundColor = ConsoleColor.White;
                                        ...
                                        Console.WriteLine($"Start: {thisCourse.schedule.startTime:hh:mmtt}");
                                        Console.WriteLine($"End: {thisCourse.schedule.endTime:hh:mmtt}\n");
                                        Console.WriteLine("--------------------------");
                                    }
                                }
                                Console.WriteLine("\n");
        ...
        public static void EditPerson(ref Person thisPerson)
        {
            ...
            if (thisPerson.GetType() == typeof(Student))
            {
                ...
                do
                {
                    // Ask for courses, must be of the form XXXX-000
                    Console.Write($"Add Courses (Enter nothing to stop)\n\t=> ");
                    string input = Console.ReadLine();
                    if (input.Length == 0)
                    {
                        break;
                    }
                    else if(!Regex.IsMatch(input, @"^[a-zA-Z]{4}-[0-9]{3}$"))
                    {
                        // Regex expression to check for valid courses
                        Console.WriteLine("\nInvalid course code: Expected (XXXX-000)");
                    }
                    else
                    {
